...
from backend.config import get_settings, POLICY_SECTIONS
...
class PolicyIndexer:
    ...
    def __init__(self):
        ...
        if self.settings.vector_db_type == "pinecone":
            ...
            if self.settings.collection_name not in existing_indexes:
                # Create index with appropriate dimension for sentence transformers
                logger.info(f"Creating Pinecone index: {self.settings.collection_name}")
            ...
            # Get index
            self.index = self.pc.Index(self.settings.collection_name)
            logger.info(f"Connected to Pinecone index: {self.settings.collection_name}")
        else:
            raise NotImplementedError(f"Vector DB type {self.settings.vector_db_type} not supported. Use 'pinecone'.")
    ...
    def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        strategy = self.settings.embedding_strategy
    ...
    def index_policy_document(self, pdf_path: str, policy_id: str, policy_name: str) -> Dict[str, Any]:
        ...
        for section in sections:
            # Chunk long sections
            chunks = self._chunk_text(section.text, self.settings.chunk_size, self.settings.chunk_overlap)
