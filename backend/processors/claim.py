...
class ClaimAnalyzer:
    ...
    def _extract_claim_items(self, extracted_data: Dict[str, Any]) -> List[ClaimItem]:
        ...
        # Normalize keys to lowercase snake_case for consistent handling
        # Groq sometimes returns "Service Information" instead of "service_info"
        normalized_data = {}
        ...
        if "servicesRendered" in extracted_data:
        ...
        elif ("service_info" in extracted_data or "service_information" in extracted_data) and \
             ("financial_info" in extracted_data or "financial_information" in extracted_data):
            # Groq response structure with nested info
            service_info = extracted_data.get("service_info", extracted_data.get("service_information", {}))
            financial_info = extracted_data.get("financial_info", extracted_data.get("financial_information", {}))
            provider_info = extracted_data.get("provider_info", extracted_data.get("provider_information", {}))
            
            # Get services from service_info
            services_list = service_info.get("services", [])
            if not services_list and "service_date" in service_info:
                # Single service case
            ...
            if services_list:
                for service in services_list:
                    items.append(ClaimItem(
                        description=service.get("service_description", service.get("description", "Medical service")),
                        code=service.get("procedure_code", service.get("code")),
                        amount=float(service.get("total_price", service.get("amount", 0))),
                        date=service.get("date", service_info.get("service_date", "")),
                        provider=provider_info.get("provider_name", provider_info.get("name", "Unknown")),
                        quantity=int(service.get("quantity", 1))
                    ))
            else:
                # Fallback to total amount from financial_info
                items.append(ClaimItem(
                    description="Medical services",
                    code=None,
                    amount=float(financial_info.get("total_amount", financial_info.get("total", 0))),
                    date=service_info.get("service_date", ""),
                    provider=provider_info.get("provider_name", provider_info.get("name", "Unknown"))
                ))
        else:
            # Fallback for simple structure
            total_amt = float(extracted_data.get("total_amount", 0))
            # Also check in financial_info if it exists
            if "financial_info" in extracted_data:
                total_amt = float(extracted_data["financial_info"].get("total_amount", total_amt))
